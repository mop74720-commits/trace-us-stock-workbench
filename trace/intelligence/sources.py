...
def timestamp(value) -> Optional[float]:
...
def fetch(url: str, headers: Optional[dict] = None) -> Tuple[int, bytes, dict]:
    validate_url(url)
    request_headers={"User-Agent":"TRACE-Intelligence/0.1 (public read-only research)","Accept-Encoding":"identity", **(headers or {})}
